...
#pragma warning restore CS0162 // 检测到无法访问的代码
    ...
    public void NavigateTo(Type? page, object? param = null, NavigationTransitionInfo? infoOverride = null)
    {
        ...
        if (destPage is null or nameof(BlankPage)
            ...
            || CurrentGameBiz.ToGame() is GameBiz.ZZZ && destPage is not nameof(GameLauncherPage) and not nameof(GameSettingPage) and not nameof(GachaLogPage) and not nameof(ScreenshotPage) and not nameof(HoyolabToolboxPage) and not nameof(SelfQueryPage))
        {
