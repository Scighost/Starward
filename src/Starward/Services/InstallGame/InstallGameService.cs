...
internal abstract class InstallGameService
{
    ...
    public abstract Task StartAsync(bool skipVerify = false);
    ...
    protected async Task PrepareForRepairAsync()
    {
        ...
        list.AddRange(await GetPkgVersionsAsync($"{separateUrlPrefix}/pkg_version").ConfigureAwait(false));
    ...
    protected async Task DownloadAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await DownloadFileAsync(task, token).ConfigureAwait(false);
            Interlocked.Increment(ref progressCount);
        }).ConfigureAwait(false);
    }
    ...
    {
        if (CurrentGameBiz is GameBiz.hk4e_cn or GameBiz.hk4e_global)
        {
            await Task.Run(() =>
            {
    ...
    /// <returns></returns>
    protected async Task DeleteDownloadedFilesAsync(IEnumerable<string> files)
    {
        await Task.Run(() =>
        {
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    protected async Task DownloadFileAsync(DownloadFileTask task, CancellationToken cancellationToken)
    {
