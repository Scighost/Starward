...
namespace Starward.Setup.Snap.HPatch;
...
public static unsafe partial class HPatch
{
    ...
    {
        var stream = new SharpCompress.Compressors.ZStandard.DecompressionStream(new InputSliceStream(codeStream, codeBegin, codeEnd), leaveOpen: false);
    ...
    private static BOOL ZstandardClose(Decompress* decompressor, nint handle)
    {
        ...
        if (gcHandle.Target is not SharpCompress.Compressors.ZStandard.DecompressionStream stream)
        {
            return true;
        }
