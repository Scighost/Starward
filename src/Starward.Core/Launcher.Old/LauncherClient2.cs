...
public class LauncherClient2
{
    ...
    public LauncherClient2(HttpClient? httpClient = null)
    {
    ...
    private async Task<T> CommonSendAsync<T>(HttpRequestMessage request, CancellationToken cancellationToken = default) where T : class
    {
        ...
        var responseData = await response.Content.ReadFromJsonAsync(typeof(miHoYoApiWrapper<T>), LauncherJsonContext2.Default, cancellationToken) as miHoYoApiWrapper<T>;
