...
namespace Starward.SevenZip
{
    ...
    internal interface IArchiveOpenCallback
    {
        ...
        void SetTotal(
            nint files, // [In] ref ulong files, can use 'ulong* files' but it is unsafe
            nint bytes); // [In] ref ulong bytes

        void SetCompleted(
            nint files, // [In] ref ulong files
            nint bytes); // [In] ref ulong bytes
    }
    ...
    */
    ...
    internal interface ISequentialOutStream
    {
        ...
        int Write(
            ...
            uint size,
            nint processedSize); // ref uint processedSize
        /*
    ...
    */
    ...
    internal interface IInStream //: ISequentialInStream
    {
        ...
        void Seek(
            long offset,
            uint seekOrigin,
            nint newPosition); // ref long newPosition
    }
    ...
    internal class StreamWrapper : IDisposable
    {
        ...
        public virtual void Seek(long offset, uint seekOrigin, nint newPosition)
        {
    ...
    internal class OutStreamWrapper : StreamWrapper, ISequentialOutStream, IOutStream
    {
        ...
        public int Write(byte[] data, uint size, nint processedSize)
        {
            BaseStream.Write(data, 0, (int)size);

            if (processedSize != nint.Zero)
            {
