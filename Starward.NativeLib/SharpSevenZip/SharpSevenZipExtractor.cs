...
/// </summary>
/// <example>
/// using (var extr = new SharpSevenZipExtractor(@"C:\Test.7z"))
/// {
///     extr.ExtractArchive(@"C:\TestDirectory");
/// }
/// </example>
public sealed partial class SharpSevenZipExtractor
        : SharpSevenZipBase, IDisposable
{
    ...
    /// Instead, use SharpSevenZipExtractor(Stream archiveStream), that constructor
    ...
    /// <param name="archiveFullName">The archive full file name.</param>
    /// <param name="format">Manual archive format setup. You SHOULD NOT normally specify it this way.
    /// Instead, use SharpSevenZipExtractor(string archiveFullName), that constructor
    /// automatically detects the archive format.</param>
    ...
    /// <param name="password">Password for an encrypted archive.</param>
    /// <param name="format">Manual archive format setup. You SHOULD NOT normally specify it this way.
    /// Instead, use SharpSevenZipExtractor(string archiveFullName, string password), that constructor
    /// automatically detects the archive format.</param>
    ...
    public SharpSevenZipExtractor(Stream archiveStream, string password)
        : this(archiveStream, password, false)
    {
    }
    ...
    /// <param name="leaveOpen">Leaves the base stream open.</param>
    /// <param name="format">Manual archive format setup. You SHOULD NOT normally specify it this way.
    /// Instead, use SharpSevenZipExtractor(Stream archiveStream, string password), that constructor
    /// automatically detects the archive format.</param>
    ...
    private void DisposedCheck()
    {
        ...
        if (_disposed)
        {
            throw new ObjectDisposedException("SharpSevenZipExtractor");
        }
#endif
    ...
    {
        return _openCallback ??= string.IsNullOrEmpty(Password)
                ...
                if (_filesCount != 0)
                {
                    ...
                    try
                    {
                        ...
                        for (uint i = 0; i < _filesCount; i++)
                        {
                            ...
                            {
                                ThrowException(null, new SharpSevenZipArchiveException("probably archive is corrupted."));
                            }
                        }
                        ...
                        for (uint i = 0; i < numProps; i++)
                        {
                            ...
                            if (PropIdToName.PropIdNames.TryGetValue(propId, out string? name))
                            {
    ...
    public event EventHandler<ExtractProgressEventArgs>? Extracting;
    ...
    /// <param name="sender">The sender of the event.</param>
    /// <param name="e">The event arguments.</param>
    private void ExtractingEventProxy(object? sender, ExtractProgressEventArgs e)
    {
    ...
    /// <exception cref="SharpSevenZipExtractionFailedException">Thrown when trying to extract from solid archives.</exception>
    public void ExtractFiles(ExtractFileCallback extractFileCallback)
    {
        ...
        if (IsSolid)
        {
            throw new SharpSevenZipExtractionFailedException("Solid archives are not supported.");
        }
