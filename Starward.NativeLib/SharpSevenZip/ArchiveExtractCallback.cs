...
internal sealed class ArchiveExtractCallback : CallbackBase, IArchiveExtractCallback, ICryptoGetTextPassword, IDisposable
{
    ...
    private long _lastReportBytesWritten;
    ...
    public ArchiveExtractCallback(IInArchive archive, string directory, int filesCount, bool directoryStructure,
        List<uint>? actualIndexes, SharpSevenZipExtractor extractor)
    {
    ...
    /// <param name="directoryStructure">The value indicating whether to preserve directory structure of extracted files.</param>
    public ArchiveExtractCallback(IInArchive archive, string directory, int filesCount, bool directoryStructure,
        List<uint>? actualIndexes, string password, SharpSevenZipExtractor extractor)
        : base(password)
    {
        Init(archive, directory, filesCount, directoryStructure, actualIndexes, extractor);
    }
    ...
    /// <param name="extractor">The owner of the callback</param>
    public ArchiveExtractCallback(IInArchive archive, Stream stream, int filesCount, uint fileIndex, SharpSevenZipExtractor extractor)
    {
    ...
    /// <param name="password">Password for the archive</param>
    /// <param name="extractor">The owner of the callback</param>
    public ArchiveExtractCallback(IInArchive archive, Stream stream, int filesCount, uint fileIndex, string password, SharpSevenZipExtractor extractor)
        : base(password)
    {
        Init(archive, stream, filesCount, fileIndex, extractor);
    }

    private void Init(IInArchive archive, string directory, int filesCount, bool directoryStructure, List<uint>? actualIndexes, SharpSevenZipExtractor extractor)
    {
    ...
    private void CommonInit(IInArchive archive, int filesCount, SharpSevenZipExtractor extractor)
    {
        _archive = archive;
    ...
    private void IntEventArgsHandler(object? sender, IntEventArgs e)
    {
        ...
        if (ts - _lastReportTimestamp < Stopwatch.Frequency / 10)
        {
            return;
        }
        long delta = _bytesWritten - _lastReportBytesWritten;
        _lastReportBytesWritten = _bytesWritten;
        _lastReportTimestamp = ts;
        Extracting?.Invoke(this, new ExtractProgressEventArgs(_bytesCount, delta, _bytesWritten));
    }
