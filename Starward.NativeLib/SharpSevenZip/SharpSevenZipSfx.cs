...
public class SharpSevenZipSfx
{
    ...
    private static SfxModule GetModuleByName(string name)
    {
        ...
        throw new SharpSevenZipSfxValidationException("The specified configuration is unsupported.");
    }
    ...
    private void LoadCommandsFromResource(string xmlDefinitions)
    {
        using var cfg = Assembly.GetExecutingAssembly().GetManifestResourceStream(
            ...
            ?? throw new SharpSevenZipSfxValidationException("The configuration \"" + xmlDefinitions + "\" does not exist.");

        using var schm = Assembly.GetExecutingAssembly().GetManifestResourceStream(
            GetResourceString(xmlDefinitions + ".xsd"))
            ?? throw new SharpSevenZipSfxValidationException("The configuration schema \"" + xmlDefinitions + "\" does not exist.");
        ...
        {
            throw new SharpSevenZipSfxValidationException(
                "\n" + validationErrors[..^1]);
        }
    ...
    private void ValidateSettings(SfxSettings settings)
    {
        ...
        if (invalidCommands.Count > 0)
        {
            ...
            throw new SharpSevenZipSfxValidationException(invalidText.ToString());
        }
    }
    ...
    public void MakeSfx(Stream archive, string sfxFileName)
    {
        ...
        MakeSfx(archive, GetDefaultSettings() ?? new(), sfxStream);
    }
