                                            ...
                                            ICryptoGetTextPassword, IDisposable
{
    ...
    private Dictionary<string, InStreamWrapper>? _wrappers = new();
    private readonly List<string> _volumeFileNames = new();
    ...
    public int GetStream(string name, out IInStream? inStream)
    {
        ...
        if (_wrappers!.TryGetValue(name, out InStreamWrapper? value))
        {
            value.Seek(0, SeekOrigin.Begin, IntPtr.Zero);
            inStream = value;
        }
        else
        {
